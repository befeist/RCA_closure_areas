...
Below you have the updated function, which was tested for the southern border.
...
## Crossing RCA lines issue

```{r}
latitude_lines <- st_transform(selected_latitude_lines, crs = 4326) 
longitude_lines <- st_transform(selected_longitude_lines, crs = 4326) 
...
isolines_testing1 <- isolines %>% 
  filter(CoordFileName == "100fm_031015.csv") %>% 
  st_combine() %>% 
  st_as_sf() %>% 
  st_set_geometry("geometry") %>% 
  mutate(boundary_type = "isobath",
...
isolines_testing2 <- isolines %>% 
  filter(CoordFileName == "150fm_020717.csv") %>% 
  st_combine() %>% 
  st_as_sf() %>% 
  st_set_geometry("geometry") %>% 
  mutate(boundary_type = "isobath",
...
selected_longitude_lines <- rbind(isolines_testing1, isolines_testing2)
```

```{r}
# constant_latitude = 32.55
# horizontal_line = st_sfc(st_linestring(matrix(c(longitude_start, constant_latitude, longitude_end, constant_latitude), ncol = 2, byrow = TRUE)), crs = 4326)
# horizontal_line_sf <- st_sf(geometry = horizontal_line) %>%
#   mutate(boundary_type = "latitude",
#          boundary_name = "34.17",
...
```{r}
testing_poly <- rca_lines_to_polygons_v5(selected_longitude_lines,selected_latitude_lines,eez_poly)
...
latitude_lines <- selected_latitude_lines
  ...
  clipped_lines <- st_intersection(longitude_lines, difference)
    ...
    leaflet() %>%
     addTiles() %>%
           addPolygons(data = bbox, color = "green", weight=1) %>%
     addPolygons(data = difference, color = "red", weight=1) %>%
     addPolylines(data = latitude_lines, color = "blue", weight=2) %>%
     addPolylines(data = clipped_lines, color = "orange", weight=2) 
  
  
  # st_write(clipped_lines, here("RCA_data_exploration","outputs","rca_poly","test.geojson"),  delete_layer = TRUE)
  # 
  # leaflet() %>%
  #    addTiles() %>%
  #     addPolygons(data = difference[4,], color = "red", weight=1) %>%
  #    addPolylines(data = latitude_lines, color = "blue", weight=2) %>%
  #     addPolylines(data = longitude_lines, color = "yellow", weight=2) %>% 
  #    addPolylines(data = clipped_lines, color = "black", weight=2) %>% 
  #    addCircleMarkers(data=points_sf, color="green")
  ...
  MultilineFromIntersections <- function(lat_line, clipped_lines) {
    ...
    if (length(latitudes) %% 2 != 0) {
      ...
      process_latitude_lines <- function(latitude_lines, latitude) {
        ## Selecting latitude line affected
        matching_indices <- find_matching_latitude_indices(latitude_lines, min_lat)
        selected_row <- latitude_lines[matching_indices,]
        ## Searching closest lines for all objects
        nearest_points <- st_nearest_points(longitude_lines,selected_row) %>%
          st_as_sf()
