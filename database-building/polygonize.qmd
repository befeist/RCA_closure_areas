...
rca_lines_sf <- rca_legis_points_df |> 
  ...
  dplyr::group_by(boundary_type, boundary_name, area_name) |>
...
```{r}
lines_sf <- rbind(coastline_merge_sf, n_s_boundaries_sf, rca_lines_sf)
...
for (site in unique(polygon_df$SiteName)) {
  ...
  fn <- stringr::str_remove(site, pattern = "TrawlRCA_")
  fn <- stringr::str_remove_all(fn, pattern = ".csv")
