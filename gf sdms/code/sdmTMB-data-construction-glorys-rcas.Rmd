...
join_glorys_trawl <- function(yr){
  ...
  out <- trawl_glorys_yr %>% 
    ...
    # change lat/lon to meters
    mutate(lattrawlUTM=lattrawlUTM*1000,lontrawlUTM=lontrawlUTM*1000,
           latglorysUTM=latglorysUTM*1000,longlorysUTM=longlorysUTM*1000)
  out
}
```
...
trawl_glorys_joined <- trawl_glorys_joined %>% 
  mutate(lontrawl_km=lontrawlUTM/1000,lattrawl_km=lattrawlUTM/1000)
```
...
```{r}
trawl_sf <- trawl_glorys_joined %>% st_as_sf(coords=c("lontrawlUTM","lattrawlUTM"),crs=26910)
# make a buffered outline of the trawl data using fmesher package
dat_buffer <- fmesher::fm_nonconvex_hull(trawl_sf,
                                concave = -0.025,
                                convex = -0.025)
# crop this buffer so it doesn't overlap the coast
dat_buffer <- st_difference(dat_buffer,summarise(coast %>% st_transform(26910)))

# plot it on top of what we just mapped
ggplot()+
  # spatial geom for maps
  geom_sf(data=trawl_sf %>% sample_n(10000))+
  geom_sf(data=coast,fill='gray20')+
  geom_sf(data=dat_buffer,fill=NA,color='red')

# GLORYS cells that touch this buffer
glorys_trawl_filt <- glorys_phy_grid %>% 
  mutate(lonUTM_m=lonUTM*1000,latUTM_m=latUTM*1000) %>% 
  st_as_sf(coords=c("lonUTM_m","latUTM_m"),crs=26910) %>% 
  st_filter(dat_buffer)

# plot again
ggplot()+
  # spatial geom for maps
  geom_sf(data=glorys_trawl_filt,color="blue",size=0.5)+
  geom_sf(data=coast,fill='gray20')+
  geom_sf(data=dat_buffer,fill=NA,color='red')

glorys_incl_cells=glorys_trawl_filt$gridID_phy
```
...
make_glorys_proj_grid<- function(yr){
  ...
  # crop- only cells that overlap with the buffer created in the last step
  filter(gridID_phy%in%glorys_incl_cells)
...
```{r}
glorys_pred_grid %>% filter(year==2003) %>% ggplot(aes(lonUTM,latUTM,color=thetao))+geom_point(size=0.5)+coord_equal()+scale_color_viridis()
```

## Raster GLORYS grid

Also make a version of this that is raster, for saving

```{r}
glorys_proj_rast <- glorys_pred_grid %>% 
  filter(year==2003) %>% 
  st_as_sf(coords=c("longitude","latitude"),crs=4326) %>% 
  dplyr::select(gridID_phy) %>% 
  st_rasterize() %>% rast() %>%
  # project to epsg 26910
  project("epsg:26910",method='near')
plot(glorys_proj_rast)
# grid numbers for matching
glorys_rast_key <- values(glorys_proj_rast,dataframe=T) %>% mutate(glorys_rastID=row_number())
```
...
```{r}
write_rds(glorys_pred_grid,here('gf sdms','data','glorys_hindcast_grid.rds'))
writeRaster(glorys_proj_rast,here('gf sdms','data','glorys_template_raster.tif'),overwrite=T)
write_rds(glorys_rast_key,here('gf sdms','data','glorys_proj_raster_key.rds'))
```
...
testbin <- test %>% mutate(incl=1) %>% select(incl)
...
# toc()
tic("terra:")
testrast <- st_rasterize(test) %>% rast()
toc()

# set NAs
template_rast <- testrast %>% 
  subst(0,NA)

# grid numbers for matching
rca_rast_key <- values(template_rast,dataframe=T) %>% mutate(rca_rastID=row_number()) %>% 
  filter(!is.na(Grid_ID))

plot(template_rast)
```

Match RCA raster to GLORYS to extract covariates

```{r}
# find nearest neighbors
t1 <- rca_cent %>% sample_n(10000) %>% 
  ggplot()+geom_point(aes(Easting_m/1000,Northing_m/1000),color='red')+coord_equal()
...
rca_base_grid <- rca_base_grid %>% 
  ...
  # remove land and make depth positive
  mutate(bathy=ifelse(bathy>0,NA,bathy)) %>% 
  mutate(bathy= -bathy)

# did this work?
tic("rasterizing bathy:")
rca_bathy <- rca_base_grid %>% 
  dplyr::select(Easting_m,Northing_m,bathy)%>% 
  st_as_sf(coords = c("Easting_m","Northing_m"),crs=26910)
br <- st_rasterize(rca_bathy) %>% rast()
# multiply by binary raster to set NAs
br <- br*template_mask
toc()
plot(br,range=c(0,1500))
```

## Save

```{r}
write_rds(rca_base_grid,here('gf sdms','data','rca_base_prediction_grid.rds'))
writeRaster(template_rast,here('gf sdms','data','rca_template_raster.tif'))
write_rds(rca_rast_key,here('gf sdms','data','rca_proj_raster_key.rds'))
```
...
```{r}
# if we want just a mean across years
mean_glorys_grid <- glorys_pred_grid %>% 
  group_by(longitude,latitude,lonUTM,latUTM,depth,bot,gridID_bgc_match,gridID_phy) %>% 
  summarise(thetao=mean(thetao),o2=mean(o2)) %>% 
  ungroup() %>%
  mutate(year=2015)%>%  # this shoulnd't matter if we don't impose any temporal structure in the model
    # rename the locaiton variables to align with var names in the model (i.e., lattrawlkm and lontrawlkm)
  mutate(lattrawl_km= latUTM,lontrawl_km=lonUTM,depth_trawl=bot)
mean_rca_grid <- rca_base_grid %>% 
  left_join(mean_glorys_grid,by=join_by(gridID_glorys_match==gridID_phy)) %>% 
  # rename the locaiton variables to align with var names in the model (i.e., lattrawlkm and lontrawlkm)
  mutate(lattrawl_km=Northing_m/1000,lontrawl_km=Easting_m/1000,depth_trawl=bathy) %>%
  filter(!is.na(bathy))
```
...
```{r}
write_rds(mean_glorys_grid,here('gf sdms','data','mean_glorys_prediction_grid.rds'))
write_rds(mean_rca_grid,here('gf sdms','data','mean_rca_prediction_grid.rds'))
```
