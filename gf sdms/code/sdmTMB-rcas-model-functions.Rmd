...
```{r}
run_sdmTMB <- function(dat,spp,use_depth=F,time_vary=F,spatial_field=T,include_substrate=F,hab_spline=F,env_spline=F,spline_k=3){
...
## Single Model Prediction (RCAs)

```{r}
# Build a prediction grid from the RCA base grid
build_rca_grid_yr <- function(yr,rcas=rca_gr,glorys=glorys_gr){
  g <- glorys %>% filter(year==yr)
  out <- rcas %>% 
    left_join(g,by=join_by(gridID_glorys_match==gridID_phy)) %>% 
...
Function to make predictions for one model on to the high-resolution RCA grid, using the "midpoint" environmental data (i.e., August GLORYS conditions)
...
make_predictions <- function(modelobj,nd){
  ...
  predicted_cpue_km2 <- predict(modelobj,newdata,return_tmb_object=T)
...
After models have been fit and projected using trawl survey and GLORYS data, we have a lot of ways to visualize the outputs.
...
Using data on the distance of each GLORYS cell from the coastline, calculate an ensemble weighted "distance from shore centroid" for each 0.1deg latitude band in each year. This function takes as input the ensemble predictions for a species (i.e, the output of `ensemble_predictions`).
